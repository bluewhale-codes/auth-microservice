...
const createStudentProfile = async (profileData, client = null) => {
  const { userId, rollNo } = profileData;
  const sql = `
    INSERT INTO student_profiles (user_id, roll_no, created_at, updated_at)
    VALUES ($1, $2, NOW(), NOW())
    RETURNING id, user_id, roll_no, created_at, updated_at
  `;
  const result = client 
    ? await client.query(sql, [userId, rollNo])
    : await query(sql, [userId, rollNo]);
  return result.rows[0];
};

const findProfileByUserId = async (userId, client = null) => {
  const sql = `SELECT * FROM student_profiles WHERE user_id = $1 LIMIT 1`;
...
module.exports = {
  createStudentProfile, findProfileByUserId, 
  checkProfileExists, checkRollNoExists
};